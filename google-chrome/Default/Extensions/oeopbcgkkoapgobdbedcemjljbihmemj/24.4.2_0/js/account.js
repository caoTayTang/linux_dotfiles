...
function Account() {
            ...
            if (!showOnlyRecentUnreadEmails || (showOnlyRecentUnreadEmails && isRecentEmailDate(issued))) {
               ...
               if (conversationView) {
                   // group emails by threadid
                    var threadFound = mailObjects.some(existingMailObject => {
            ...
            getGmailAtPromise = async function() {
                ...
                if (!gmailAT || lastGmailATFetch.diffInMinutes() <= -5 || force) {
                    await new Promise((resolve, reject) => {
                        chrome.cookies.get({name: 'GMAIL_AT', url: `https://mail.google.com/mail/u/${that.id}`}, response => {
                            gmailAT = response?.value;
                            if (gmailAT) {
                                resolve();
                            } else {
                                reject("No gmail AT found");
                            }
                        });
                    });
   ...
   async function fetchGmailAT1() {
        let data;
        try {
            data = await ajaxBasicHTMLGmail(that.getMailUrl({useBasicGmailUrl:true}) + Math.ceil(1000000 * Math.random()), {fetchGmailAT: true});
        } catch (error) {
            throw "gmail AT error: " + error;
        }

        // must keep the \b because some emails had content like ... format=123  and the at=123 was matched!
        const matches = data.match(/\bat\=([^\"\&]*)/); //  new one:   /at\=([^\"\&]*)/     old one: /\at=([^"&]+)/
        if (matches?.length) { // sample at: AF6bupND7QvFVgjkbg_PAWf7jUnB-zQFTQ   xF6bupMdRlq_g8bP0qwnUMLHR3_PwMA7PA  xF6bupMQ3nNhcKWmba7nVIHX8Am4WaH_qQ  
            let foundAT;
            for (let a=1; a<matches.length; a++) {
