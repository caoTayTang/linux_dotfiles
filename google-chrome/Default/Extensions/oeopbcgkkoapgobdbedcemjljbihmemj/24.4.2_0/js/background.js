...
if (chrome.alarms) {
    chrome.alarms.onAlarm.addListener(async alarm => {
        ...
        if (alarm.name == Alarms.EVERY_MINUTE) {
            ...
            //sendOptionStat("previewing_marks_as_read");
            ...
            accounts.forEach(account => {
                account.fetchSendAs().catch(error => {
                    console.warn("sync fetchSendAs() error: " + error);
                });
            });
        ...
        if (dndState) {
            ...
            if (passedDoNotShowNotificationIfGmailTabOpenTest) {
                ...
                if (textNotification || !chrome.notifications) {
                    ...
                    if (!firstEmail.authorMail.includes("google.")) {
                        globalThis.otpCode = firstEmail.getOTPCode();
                    }
    ...
    chrome.webRequest.onCompleted.addListener(
        async function(details) {
            ...
            if (await storage.get("voiceInput")) {
                ...
                if (details.url?.includes("https://mail.google.com/mail/mu/")) {
...
function init() {
        ...
        chrome.storage.local.get(["detectedChromeVersion"], result => {
            if (!result.detectedChromeVersion) {
                ...
                DetectClient.getChromeChannel().then(result => {
                    if (result?.channel != "stable" && result?.channel != "extended") {
                        const notification = new Notification("You are not using the stable channel of Chrome", {
                            body: "Click for more info. Bugs might occur, you can use this extension, however, for obvious reasons, these bugs and reviews will be ignored unless you can replicate them on stable channel of Chrome.",
                            icon: Icons.NOTIFICATION_ICON_URL
                        });
                        notification.onclick = function () {
	...
	storagePromise.then(async () => {
        ...
        lastNotificationAccountDates = deepClone(await storage.get("_lastNotificationAccountDates"));
...
async function mailUpdate(params = {}) {
    ...
    if (newEmailUpdate && params.source != "popup") {
        chrome.runtime.sendMessage({command: "newEmailUpdate", account: accountWithNewestMail.getEmail(), id: mostRecentNewEmail.id});
    }
}
			...
			if (dndState || source == "") {
				...
				if (DetectClient.isLinux() || changedSrc) {
					if (source.indexOf("custom_") == 0) {
                        ...
                        console.log("loadin audio src2", notificationAudio.src)
					}
               }
