...
class Mail {
    constructor() {
        ...
        this.getOTPCode = function () {
            const subject = that.title;
            const emailContent = that.getLastMessageText({htmlToText:true});

            if (subject?.match(/\bcode\b/i) || subject?.match(/\bpasscode\b/i) || emailContent?.match(/\bsecurity code|one\-time passcode\b/i)) {
                const otpCodeMatch = emailContent.match(/\b([0-9]{6,8})\b/g);
                if (otpCodeMatch) {
